...
func (e *DockercfgDeletedController) removeDockercfgSecretReference(dockercfgSecret *v1.Secret) error {
	...
	if changed {
		_, err = e.client.CoreV1().ServiceAccounts(dockercfgSecret.Namespace).Update(context.TODO(), serviceAccount, metav1.UpdateOptions{})
		if err != nil {
			return err
		}
	}
...
func (e *DockercfgDeletedController) getServiceAccount(secret *v1.Secret) (*v1.ServiceAccount, error) {
	...
	serviceAccount, err := e.client.CoreV1().ServiceAccounts(secret.Namespace).Get(context.TODO(), saName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
