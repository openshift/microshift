		...
		for _, containerType := range []string{"regular", "non-sidecar-init", "sidecar"} {
			...
			t.Run(fmt.Sprintf("%s/containerType=%s", tt.name, containerType), func(t *testing.T) {
				...
				for _, p := range []*v1.Pod{testPod1, testPod2, testPod3} {
					require.NoError(t, allocationManager.SetAllocatedResources(p))
				}
				...
				t.Cleanup(func() {
					for _, p := range []*v1.Pod{testPod1, testPod2, testPod3} {
						allocationManager.RemovePod(p.UID)
					}
				})
...
func TestNonAllocatedPodsExcludedFromCapacity_Resize(t *testing.T) {
	allocationManager, runningPod, pendingPod := setupNonAllocatedCapacityTest(t)
...
func TestNonAllocatedPodsExcludedFromCapacity_AddPod(t *testing.T) {
	...
	ok, reason, message := allocationManager.AddPod([]*v1.Pod{runningPod, pendingPod}, newPod)
...
func setupNonAllocatedCapacityTest(t *testing.T) (Manager, *v1.Pod, *v1.Pod) {
	t.Helper()
	if goruntime.GOOS == "windows" {
		t.Skip("InPlacePodVerticalScaling is not currently supported for Windows")
	}
	...
	return allocationManager, runningPod, pendingPod
}
