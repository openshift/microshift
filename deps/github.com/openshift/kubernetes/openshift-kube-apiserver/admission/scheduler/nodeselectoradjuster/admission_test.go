	...
	tests := []struct {
		...
		},
		{
			...
			pod: makePod(
				withNamespace(vpaOperatorNamespace),
				withLabels(map[string]string{vpaOperatorLabelKey: vpaOperatorLabelValue}),
				withNodeSelector(map[string]string{masterRoleKey: "", "topology.kubernetes.io/zone": "us-east-1a"}),
			),
			resource: coreapi.Resource("pods").WithVersion("v1"),
			expectedNodeSelector: map[string]string{
				masterRoleKey:                 "",
...
func TestRequiresNodeSelectorAdjustment(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			pod:      makePod(withNamespace(vpaOperatorNamespace), withLabels(map[string]string{vpaOperatorLabelKey: vpaOperatorLabelValue}), withNodeSelector(map[string]string{masterRoleKey: "", "extra": "value"})),
			expected: false,
		},
		{
			name:     "VPA operator in wrong namespace: no match",
			pod:      makePod(withNamespace("other-namespace"), withLabels(map[string]string{vpaOperatorLabelKey: vpaOperatorLabelValue}), withNodeSelector(map[string]string{masterRoleKey: ""})),
			expected: false,
		},
		{
