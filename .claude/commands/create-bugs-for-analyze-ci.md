...
# create-bugs-for-analyze-ci

## Synopsis
```bash
/create-bugs-for-analyze-ci <release> [--create]
```
...
This command does NOT re-analyze CI jobs. It consumes the existing `/tmp/analyze-ci-release-<release>-job-*.txt` files that were previously generated by `analyze-ci-for-release`.
...
- Job analysis files must already exist at `/tmp/analyze-ci-release-<release>-job-*.txt`
...
3. Glob for job files: `/tmp/analyze-ci-release-<release>-job-*.txt`
...
2. **Construct the bug description** using Jira wiki markup:
   ```text
   ...
   # Run the periodic CI job(s) listed below
   # Observe failure in step: <step_name>
   ...
   *Source:* Auto-generated by /create-bugs-for-analyze-ci from analyze-ci-for-release <release> output.
   ```
...
**Actions**:
1. Save report to `/tmp/file-bugs-analyze-ci-<release>-<timestamp>.txt`
2. Display summary to user:

**Dry-run report format**:
```text
===============================================================
FILE BUGS FOR ANALYZE-CI - DRY-RUN REPORT
Release: <release>
Date: YYYY-MM-DD
===============================================================
...
Report saved: /tmp/file-bugs-analyze-ci-<release>-<timestamp>.txt
===============================================================
```
...
```text
===============================================================
FILE BUGS FOR ANALYZE-CI - CREATION REPORT
Release: <release>
Date: YYYY-MM-DD
===============================================================
...
```bash
/create-bugs-for-analyze-ci 4.22
```
Shows what bugs would be created without creating anything.

### Example 2: Create Bugs
```bash
/create-bugs-for-analyze-ci 4.22 --create
```
Interactively creates bugs, prompting for each candidate.

### Example 3: No Job Files Found
```bash
/create-bugs-for-analyze-ci 4.19
```
```text
Error: No job analysis files found at /tmp/analyze-ci-release-4.19-job-*.txt
