...
cmd_prepare() {
    ...
    for release in "${RELEASES[@]}"; do
        ...
        raw_err=$(mktemp)
        if ! raw_json=$(bash "${SCRIPT_DIR}/microshift-prow-jobs-for-release.sh" "${release}" 2>"${raw_err}"); then
    ...
    if ${do_rebase}; then
        ...
        pr_err=$(mktemp)
        if ! pr_json=$(bash "${SCRIPT_DIR}/microshift-prow-jobs-for-pull-requests.sh" \
