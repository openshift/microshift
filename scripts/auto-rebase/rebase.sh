...
EOF
    ...
    yq -i '.spec.template.spec.containers[0].env += {"name": "ROUTER_DEFAULT_TUNNEL_TIMEOUT", "value": "{{ .RoTunnelTimeoututerMode }}"}' "${REPOROOT}"/assets/components/openshift-router/deployment.yaml
