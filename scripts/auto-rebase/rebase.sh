...
EOF
    ...
    yq -i '.spec.template.spec.initContainers[0].image = "{{ .ReleaseImage.haproxy_router_haproxy32 }}"' "${REPOROOT}"/assets/components/openshift-router/deployment.yaml
    ...
    yq -i '.spec.template.spec.containers[0].image = "{{ .ReleaseImage.haproxy_router_haproxy32 }}"' "${REPOROOT}"/assets/components/openshift-router/deployment.yaml
