...
# Setup rootless podman when running in a CI container where the
# current user may not have a passwd entry or subuid/subgid ranges.
...
if [ ! -f "${HOME}/.config/containers/storage.conf" ] && [ ! -d "${HOME}/.local/share/containers/storage" ]; then
