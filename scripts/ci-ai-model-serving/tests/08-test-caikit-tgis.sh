...
CAIKIT_TGIS_IMAGE="$(jq -r '.images | with_entries(select(.key == "caikit-tgis-image")) | .[]' /usr/share/microshift/release/release-ai-model-serving-"$(uname -i)".json)"
TGIS_IMAGE="$(jq -r '.images | with_entries(select(.key == "tgis-image")) | .[]' /usr/share/microshift/release/release-ai-model-serving-"$(uname -i)".json)"
...
# curl -kL -H 'Content-Type: application/json' -d '{"model_id": "flan-t5-small-caikit", "inputs": "At what temperature does Nitrogen boil?"}' flan-t5-predictor.apps.example.com/api/v1/task/text-generation | jq
