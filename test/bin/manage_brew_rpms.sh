...
action_download() {
    ...
    case ${ver_type} in
        zstream)
            package_y0="$(find_package "${ver}" "${ver_type}" "0" "0")"
            package_z1="$(find_package "${ver}" "${ver_type}" "0" "1")"
            package_y1="$(find_package "${ver}" "${ver_type}" "1" "0")"
            package_y2="$(find_package "${ver}" "${ver_type}" "2" "0")"
            package_list=("${package_y0}" "${package_y1}" "${package_y2}" "${package_z1}")
            ;;
        nightly|rc|ec)
