...
AWS_BUCKET_NAME="microshift-build-cache-ushift-6205-us-west-2"
BCH_SUBDIR=
TAG_SUBDIR=
...
action_download() {
    ...
    for dir in mirror-registry brew-rpms repo ; do
        ...
        echo "Downloading data from '${pkg_src}'"
        rm -f "${pkg_dst}"
        run_aws_cli s3 cp --quiet "${pkg_src}" "${pkg_dst}"

        # Unarchive the files after the download
        rm -rf "${pkg_dir}"
        tar xf "${pkg_dst}" -C "${dst_base}"
        rm -f "${pkg_dst}"

        local repo_size
        repo_size="$(du -csh "${pkg_dir}" | awk 'END{print $1}')"
