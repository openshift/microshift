...
scenario_create_vms() {
    ...
    prepare_kickstart host1 kickstart-bootc.ks.template "${start_image}" true true
...
EOF"
    ...
    echo "INFO: Waiting for MicroShift to be ready..."
    local vmname="host1"
    ...
    # Run all tests except version.robot (which fails due to expected config file modifications)
    # Config files like /etc/tuned/microshift-baseline-variables.conf are customized in low-latency scenarios
