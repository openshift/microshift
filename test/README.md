...
```
$ ./test/run.sh -h
run.sh [-h] [-n] [-o output_dir] [-v venv_dir] [-i var_file] [-s name-value] [test suite files]
...
Stress Testing consists on setting restricted conditions before running Tests.
The conditions consist of limiting resources at OS level (latency, bandwidth,
packet loss, memory, disk...). This conditions can be enabled and disabled
in a local or remote host. Use `./test/bin/stress_testing.sh` script to 
enable/disable conditions.

### Stress Testing with Test Scenarios

Use `stress_testing` function from `./test/bin/scenario.sh` to enable and disable
Stress Condition. Example: `stress_testing host1 enable latency 100`
