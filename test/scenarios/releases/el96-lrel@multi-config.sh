...
EOF"
    ...
    run_command_on_vm host1 'bash -s' < "${SCENARIO_DIR}/../../../scripts/lvms-helpers/createWorkloads.sh"

    echo "INFO: Checking LVMS resources..."
    run_command_on_vm host1 'bash -s' < "${SCENARIO_DIR}/../../../scripts/lvms-helpers/checkLvmsResources.sh"

    # Run ginkgo tests to validate all configurations work together
    echo "INFO: Running ginkgo validation tests for multi-config scenario..."
    ...
    run_command_on_vm host1 'bash -s' < "${SCENARIO_DIR}/../../../scripts/lvms-helpers/checkWorkloadExists.sh"

    # Cleanup LVMS workloads
    echo "INFO: Cleaning up LVMS workloads..."
    run_command_on_vm host1 'bash -s' < "${SCENARIO_DIR}/../../../scripts/lvms-helpers/cleanupWorkload.sh"
