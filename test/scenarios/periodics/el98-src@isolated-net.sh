...
scenario_create_vms() {
    prepare_kickstart host1 kickstart-isolated.ks.template rhel-9.8-microshift-source-isolated
    # Use the isolated network when creating a VM
    launch_vm --boot_blueprint rhel-9.8 --network "${VM_ISOLATED_NETWORK}"
}
