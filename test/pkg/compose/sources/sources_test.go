...
import (
	...
	"github.com/openshift/microshift/test/pkg/util/mocks"
...
{{- end -}}`)},
	...
	opts := &SourceConfigurerOpts{
		Composer: composer,
		TplData: templatingdata.NewShim(&templatingdata.TemplatingData{
			Arch:         arch,
			RHOCPMinorY:  rhocpY,
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-base", &util.Event{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-crel", &util.Event{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-external", &util.Event{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-local", &util.Event{}))).Once()

	// microshift-prel - should not cause AddSource to be called, but AddEvent with SkippedEvent should be called
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-prel", &util.SkippedEvent{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("rhocp-y1", &util.Event{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("rhocp-y2", &util.Event{}))).Once()
...
{{- end -}}`)},
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-base", &util.FailedEvent{}))).Once()
	...
	events.On("AddEvent", mock.MatchedBy(EventShouldBe("microshift-crel", &util.FailedEvent{}))).Once()
...
func EventShouldBe(expectedName string, expectedType util.IEvent) func(util.IEvent) bool {
	return func(ev util.IEvent) bool {
