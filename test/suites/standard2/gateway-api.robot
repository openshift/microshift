...
Create Gateway
    ...
    Oc Wait    -n ${namespace} gateway/test-gateway --for="condition=Accepted" --timeout=60s
    Oc Wait    -n ${namespace} deploy test-gateway-openshift-gateway-api --for=condition=Available --timeout=60s
    Oc Wait    -n ${namespace} gateway/test-gateway --for="condition=Programmed" --timeout=60s
...
Create HTTP Route
    ...
    Oc Apply    -n ${namespace} -f ${tmp}
    Oc Wait
    ...    -n ${namespace} httproutes/http --for jsonpath='{.status.parents[].conditions[?(@.type=="Accepted")].status}=True' --timeout=60s
    Oc Wait
    ...    -n ${namespace} httproutes/http --for jsonpath='{.status.parents[].conditions[?(@.type=="ResolvedRefs")].status}=True' --timeout=60s
