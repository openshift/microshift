...
Missing Bin Directory Prevents Start
    ...
    Pattern Should Appear In Log Output    ${CURSOR}    imageCredentialProviderBinDir.*file or directory does not exist
...
World Writable Bin Directory Prevents Start
    ...
    Pattern Should Appear In Log Output
    ...    ${CURSOR}
    ...    imageCredentialProviderBinDir.*must be owned by root and not writable by group or others
...
Empty Configuration Directory Prevents Start
    ...
    Pattern Should Appear In Log Output
    ...    ${CURSOR}
    ...    imageCredentialProviderConfigPath.*directory contains no .json, .yaml, or .yml configuration files
...
Teardown
    ...
    # Defensive: a failed test may leave the config dir behind; remove it too.
    Command Should Work    rm -rf ${CP_BIN_DIR} ${CP_CONFIG_FILE} ${CP_CONFIG_DIR}
    Restart MicroShift
    Remove Kubeconfig
