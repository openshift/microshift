...
Router Verify Security Configuration
    ...
    Pod Environment Should Match Value    openshift-ingress    ROUTER_MUTUAL_TLS_AUTH_CA    /etc/pki/tls/client-ca/ca-bundle.pem
    Pod Environment Should Match Value    openshift-ingress    ROUTER_CIPHERS    ECDHE-RSA-AES256-GCM-SHA384:DHE-RSA-AES256-GCM-SHA384
...
Create Custom Resources
    ...
    Run With Kubeconfig    oc get secret router-certs-default -n openshift-ingress -oyaml | sed 's/name: .*/name: router-certs-custom/' | oc create -f - -oyaml | true
    Run With Kubeconfig    oc extract configmap/openshift-service-ca.crt --to=/tmp/ --confirm 
    Run With Kubeconfig    oc create configmap router-ca-certs-custom -n openshift-ingress --from-file=ca-bundle.pem=/tmp/service-ca.crt --dry-run -o yaml | oc apply -f -
