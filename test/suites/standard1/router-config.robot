...
#    Pod Environment Should Match Value    openshift-ingress    ROUTER_DONT_LOG_NULL    2s
#    Pod Environment Should Match Value    openshift-ingress    ROUTER_HTTP_IGNORE_PROBES    2s
#    Pod Environment Should Match Value    openshift-ingress    ROUTER_DISABLE_HTTP2    2s
...
Pod Environment Should Match Value
    ...
    # run the command
    # oc get pods -n openshift-ingress -o=jsonpath='{.items[*].spec.containers[*].env[?(@.name=="ROUTER_BUF_SIZE")].value}'