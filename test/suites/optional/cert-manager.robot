...
Test Cert manager with local acme server
    ...
    Log    Retrieving DNS name from MicroShift config
    ${dns_name}=    Get DNS Name From MicroShift Config
    Log    Using DNS name: ${dns_name}

    Log    Creating ACME issuer with HTTP01 solver
    ...
    Log    Verifying issued certificate content
...
Setup Pebble Server
    ...
    [Arguments]    ${namespace}
    Log    Setting up Pebble server in namespace: ${namespace}

    Log    Creating namespace if it doesn't exist
    Create Namespace If Not Exists    ${namespace}

    Log    Creating deployment and service for Pebble server from file
...
Verify Certificate Common Name
    ...
    # Note: Simplified validation - trusts cert-manager created certificate correctly

Cleanup HTTP01 Resources
    [Documentation]    Cleanup HTTP01 test resources
    Log    Cleaning up HTTP01 resources
