...
Logs Should Contain Exported Log Data
    [Documentation]    OpenTelemetry logs should not contain errors by any component (reciever, processor,exporter).
...
Logs Should Not Contain Reciever Errors
    [Documentation]    Internal reciever errors are not treated as fatal. Typically these are due to a miconfiguration
