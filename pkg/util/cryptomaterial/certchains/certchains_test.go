		...
		NewCertificateSigner("test-signer1", filepath.Join(tmpDir, "test-signer1"), 30).
			...
			NewCertificateSigner("test-signer1-subca", filepath.Join(tmpDir, "test-signer1", "intemediateDir", "subca"), 30).
				...
				NewCertificateSigner("test-signer1-subca-too", filepath.Join(tmpDir, "test-signer1", "intemediateDir", "subca", "subca-too"), 30).
					...
					NewCertificateSigner("test-signer1-subca-too-too2", filepath.Join(tmpDir, "test-signer1", "intemediateDir", "subca", "subca-too", "subca-too-too2"), 12),
				),
			),
		),
...
func TestWhenToRotateAtEarliest(t *testing.T) {
	...
	require.True(t, time.Now().Add(6*24*time.Hour).Before(rotationTime) && time.Now().Add(8*24*time.Hour).After(rotationTime))
}
