...
func (c *AtomicDirCopy) CopyToIntermediate() error {
	...
	if err != nil {
		...
		if err := os.RemoveAll(c.intermediatePath); err != nil {
			return errors.Join(copyErr, fmt.Errorf("failed to remove intermediate path %q: %w", c.intermediatePath, err))
		}
		return copyErr
	}
...
func GenerateUniqueTempPath(path string) (string, error) {
	...
	return "", fmt.Errorf("attempted to generate unique temporary path for 1000 tries - giving up")
}
