...
func (s *KubeAPIServer) configure(cfg *config.Config) error {
	...
	if len(cfg.ApiServer.NamedCertificates) > 0 {
		for _, namedCertsCfg := range cfg.ApiServer.NamedCertificates {
			...
			//Validate the cert is non-distructive
			certAllowed, err := util.IsCertAllowed(cfg.Network.ClusterNetwork, cfg.Network.ServiceNetwork, namedCertsCfg.CertPath, namedCertsCfg.Names)
			if err != nil {
				klog.Warningf("failed to read NamedCertificates: %v, certificate ignored", err)
				continue
			}

			if !certAllowed {
				klog.Warningf("Certificate %v not allowed and ignored", namedCertsCfg)
				continue
			}
