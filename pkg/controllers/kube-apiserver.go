...
func (s *KubeAPIServer) configure(cfg *config.MicroshiftConfig) error {
	...
	if err := s.configureOAuth(cfg); err != nil {
		return fmt.Errorf("Failed to configure kube-apiserver OAuth %w", err)
	}
...
func (s *KubeAPIServer) Run(ctx context.Context, ready chan<- struct{}, stopped chan<- struct{}) error {
	...
	fd, err := os.CreateTemp("", "kube-apiserver-config-*.yaml") // todo: path
	if err != nil {
		return err
	}
	defer os.Remove(fd.Name()) // todo: error ignored
	err = func() error {
		defer fd.Close()
